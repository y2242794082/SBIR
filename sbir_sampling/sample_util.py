...
def make_negative_list(triplets):
    ...
    # all_inds = np.unique(triplets)
    all_inds = np.array([i for i in range(num_images)])
    neg_list = []
    for i in range(num_images):
        # pos_inds = np.union1d(tri_mat[i, :, 0], tri_mat[i, :, 1])
        pos_inds = tri_mat[i][0]
        for j in range(len(tri_mat[i])):
            pos_inds = np.union1d(tri_mat[i][j], pos_inds)
            
        # print(pos_inds)
        neg_inds = np.setdiff1d(all_inds, pos_inds).reshape([1, -1])
        while len(neg_inds[0]) > num_images - 10:
            neg_inds = np.delete(neg_inds, 0, axis=1)
