...
class MemoryBlockManager:
    ...
    def pop_batch_circular(self, this_batch_size):#对其数组索引使用sample_ind（数组）存在疑问,但程序中并未使用该函数,无法验证
        sample_ind = self.pop_batch_inds_circular(this_batch_size)
        #print ('sample_ind:', sample_ind)
        data = self.batch_data[sample_ind, ::]
        #print ('batch_data:', self.batch_data)
        #print ('data:', data)
        if self.has_label:
    ...
    def pop_batch_inds_circular(self, this_batch_size):#超出范围的下标从头开始继续取
