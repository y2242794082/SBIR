    ...
    with open('/proc/driver/nvidia/gpus/%d/information' % gpu) as f:
        for line in f:
            if line.startswith('Bus Location'):
                ...
                ff.close()
                cpus = [cpu for s in cpus_str.split(',') for cpu in
                        range(int(s.split('-')[0]), int(s.split('-')[1]) + 1)]
                return cpus
    return [-1]
